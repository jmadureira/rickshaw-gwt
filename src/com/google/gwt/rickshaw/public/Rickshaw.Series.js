...
Rickshaw.Series.prototype.removeSeries = function(data) {
	var self = this;
	data.forEach( function (name) {
	    var ind = self.indexOf(self.itemByName(name));
	    console.log("Removing series " + name + " on index " + ind);
	    self.splice(ind,1);
	    if (self.legend) {
		self.legend.removeLine(name);
	    }
	});
}
